...
model = yolo("yolov8n-pose_openvino_model", task="pose")  ## defining the model

cap = cv.VideoCapture(0)  ## to use a video from the device
# cap = cv.VideoCapture("output1.avi")  ## To capture from the webcam
...
while cap.isOpened():
    ...
    if ret:
        ...
        for j in range(len(joints)): ### This loop ensures to detect pose for more than one persons.
            ...
            # print(out) ######### print statement for debugging
            final = pose(out) ### text variable containg the pose names.
            cv.putText(
            img_text,
            text="Person, id:"+str(j+1),
